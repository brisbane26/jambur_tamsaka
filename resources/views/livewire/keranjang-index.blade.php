    ...
    <div class="flex flex-col items-center justify-center gap-1">
        @if(in_array($keranjang->paket->kategori->nama_kategori, ['Salon', 'Musik', 'Dekorasi', 'Gedung', 'Dokumentasi', 'Lainnya']))
            <input
                ...
                value="1"
                readonly
                class="w-20 text-center border rounded bg-gray-100 cursor-not-allowed">
        @else
            <input
                type="number"
                ...
                min="50"
                max="2000"
                wire:input.debounce.100ms="updateQuantityLive({{ $keranjang->id }}, $event.target.value)"
                class="w-20 text-center border rounded"
                onchange="validateQuantity(this, {{ $keranjang->id }})"
            />
            <p id="error-{{ $keranjang->id }}" class="text-red-500 text-sm mt-1 hidden">
                Kuantitas harus antara 50 - 2000
            </p>
        @endif
    </div>
</td>
