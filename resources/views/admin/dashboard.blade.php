        ...
        <div class="flex flex-wrap -mx-6">
            ...
            <div class="w-full mt-6 px-6 sm:w-1/2 xl:w-1/3 sm:mt-0">
                <div class="flex items-center px-5 py-6 shadow-sm rounded-md bg-white">
                    ...
                    <div class="mx-5">
                        <h4 class="text-2xl font-semibold text-gray-700">{{ App\Models\Pesanan::count() }}</h4>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @php
                            $topPaket = 
                                \App\Models\Paket::select('pakets.id', 'pakets.nama_paket', \DB::raw('COUNT(detail_pesanans.id) as total'))
                                ->leftJoin('detail_pesanans', 'pakets.id', '=', 'detail_pesanans.paket_id')
                                ->leftJoin('pesanans', 'detail_pesanans.pesanan_id', '=', 'pesanans.id')
                                ->whereNotNull('pesanans.id')
                                ->groupBy('pakets.id', 'pakets.nama_paket')
                                ->orderByDesc('total')
                                ->limit(5)
                                ->get();
                        @endphp
                        @foreach($topPaket as $i => $paket)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $i+1 }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $paket->nama_paket }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $paket->total }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
